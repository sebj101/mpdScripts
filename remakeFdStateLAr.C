...
void remakeFdStateLAr(bool isFhc, bool isWeighted, bool isCC,
		   ...
		   const char* cafs="/dune/data/users/sbjones/CAFs/nuwroRW/withEAvail/")
{
  ...
  RemoveSysts(systlist, {"NuWroRWT", "NuWroQ2", "NuWroW", "NuWroRWTLar", "NuWroQ0Q3", "NuWroQ0Q3CC"});

  for (unsigned int i=0; i<systlist.size(); i++) {
    std::cout<<systlist.at(i)->ShortName()<<std::endl;
  }

  SystShifts ndWeight(systlist.at(systlist.size()-2), 1);
  SystShifts ndWeightCC(systlist.at(systlist.size()-3), 1);
  
  std::cout<<"ndweight CC syst is "<<systlist.at(systlist.size()-3)->ShortName()<<std::endl;
  std::cout<<"ndweight syst is "<<systlist.at(systlist.size()-2)->ShortName()<<std::endl;  
  ...
  if (isFhc) {
    ...
    if (isWeighted) {
      if (isCC) {
 	PredictionInterp predNumuFHCreco_wgt(systlist, this_calc, gennumureco, loadersFHC, ndWeightCC); 
        PredictionInterp predNueFHCreco_wgt(systlist, this_calc, gennuereco, loadersFHC, ndWeightCC); 
	loadersFHC.Go();

	TFile *fFDfhc = new TFile(Form("%s/state_FD_FHC_lar_wgt_cc.root", stateFileDir), "recreate"); 
	std::cout<<"Saving weighted FD numu FHC to "<<Form("%s/state_FD_FHC_lar_wgt_cc.root", stateFileDir)<<std::endl;
	predNumuFHCreco_wgt.SaveTo(fFDfhc->mkdir("fd_interp_numu_fhc_wgt"));
	std::cout<<"Saving weighted FD nue FHC to "<<Form("%s/state_FD_FHC_lar_wgt_cc.root", stateFileDir)<<std::endl;
	...
	TFile *fFDfhc = new TFile(Form("%s/state_FD_FHC_lar_wgt.root", stateFileDir), "recreate"); 
	std::cout<<"Saving weighted FD numu FHC to "<<Form("%s/state_FD_FHC_lar_wgt.root", stateFileDir)<<std::endl;
	predNumuFHCreco_wgt.SaveTo(fFDfhc->mkdir("fd_interp_numu_fhc_lar_wgt"));
	std::cout<<"Saving weighted FD nue FHC to "<<Form("%s/state_FD_FHC_lar_wgt.root", stateFileDir)<<std::endl;
	predNueFHCreco_wgt.SaveTo(fFDfhc->mkdir("fd_interp_nue_fhc_lar_wgt"));
	fFDfhc->Close();
	delete fFDfhc;
	std::cout<<"Made & saved all PredictionInterps"<<std::endl;
      }      
    ...
    if (isWeighted) {
      if (isCC) {
	PredictionInterp predNumuRHCreco_wgt(systlist, this_calc, gennumureco, loadersRHC, ndWeightCC); 
	PredictionInterp predNueRHCreco_wgt(systlist, this_calc, gennuereco, loadersRHC, ndWeightCC);
	loadersRHC.Go();

	TFile *fFDrhc = new TFile(Form("%s/state_FD_RHC_lar_wgt_cc.root", stateFileDir), "recreate");
	std::cout<<"Saving weighted FD numu RHC to "<<Form("%s/state_FD_RHC_lar_wgt_cc.root", stateFileDir)<<std::endl;
	predNumuRHCreco_wgt.SaveTo(fFDrhc->mkdir("fd_interp_numu_rhc_lar_wgt"));
	std::cout<<"Saving weighted FD nue RHC to "<<Form("%s/state_FD_RHC_lar_wgt_cc.root", stateFileDir)<<std::endl;
	predNueRHCreco_wgt.SaveTo(fFDrhc->mkdir("fd_interp_nue_rhc_lar_wgt"));
	fFDrhc->Close();
	delete fFDrhc;
	std::cout<<"Made & saved all PredictionInterps"<<std::endl;
      }
	...
	TFile *fFDrhc = new TFile(Form("%s/state_FD_RHC_lar_wgt.root", stateFileDir), "recreate");
	std::cout<<"Saving weighted FD numu RHC to "<<Form("%s/state_FD_RHC_lar_wgt.root", stateFileDir)<<std::endl;
	predNumuRHCreco_wgt.SaveTo(fFDrhc->mkdir("fd_interp_numu_rhc_lar_wgt"));
	std::cout<<"Saving weighted FD nue RHC to "<<Form("%s/state_FD_RHC_lar_wgt.root", stateFileDir)<<std::endl;
	predNueRHCreco_wgt.SaveTo(fFDrhc->mkdir("fd_interp_nue_rhc_lar_wgt"));
	fFDrhc->Close();
	delete fFDrhc;
	std::cout<<"Made & saved all PredictionInterps"<<std::endl;
